...
func commandsRun(commands [][]string) {
	for _, command := range commands {
		...
		case "createuhiddevices":
			...
			} else if len(command) == 7 {
				if command[1] != "" {
					if !inputUhidCreateDevice(command[1], 0x01, command[4], controlSocket) {
						return
					}
				}

				if command[2] != "" {
					if !inputUhidCreateDevice(command[2], 0x02, command[5], controlSocket) {
						return
					}
				}

				if command[3] != "" {
					if !inputUhidCreateDevice(command[3], 0x03, command[6], controlSocket) {
						return
					}
				}
			} else {
