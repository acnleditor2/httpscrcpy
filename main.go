				...
				if connect {
					...
					if config.Scrcpy.Control {
						if config.Scrcpy.UhidKeyboardReportDesc != "" {
							if !inputUhidCreateDevice(config.Scrcpy.UhidKeyboardReportDesc, 0x01, config.Scrcpy.UhidKeyboardName, controlSocket) {
						...
						if config.Scrcpy.UhidMouseReportDesc != "" {
							if !inputUhidCreateDevice(config.Scrcpy.UhidMouseReportDesc, 0x02, config.Scrcpy.UhidMouseName, controlSocket) {
								go func() { connectionControlChannel <- false }()
								continue
							}
						}
						...
						if config.Scrcpy.UhidGamepadReportDesc != "" {
							if !inputUhidCreateDevice(config.Scrcpy.UhidGamepadReportDesc, 0x03, config.Scrcpy.UhidGamepadName, controlSocket) {
								go func() { connectionControlChannel <- false }()
								continue
							}
						}
